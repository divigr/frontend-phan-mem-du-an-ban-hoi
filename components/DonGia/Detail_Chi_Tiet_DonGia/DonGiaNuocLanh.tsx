...
const DonGiaNuocLanh: React.FC<Props> = ({ projectId, filterDate }) => {
  ...
  const loadData = async (append = false) => {
    ...
    try {
      ...
      const formattedData = newData.flatMap((item: any) =>
        item.nuocLanh.map((nuocLanh: any) => ({
    ...
    <div>
      <SharedTable
        columns={[
          ...
          { key: 'luongSuDung', label: 'Lượng Tiêu Thụ (m³)' },
          { key: 'donGia', label: 'Đơn Giá Nước Lạnh (VND)' },
