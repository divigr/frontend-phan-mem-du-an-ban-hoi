...
const DonGiaGas: React.FC<Props> = ({ projectId, filterDate }) => {
    ...
    <div>
      <SharedTable
        columns={[
          ...
          { key: 'luongTieuThu', label: 'Lượng Tiêu Thụ (kg)' },
          { key: 'donGia', label: 'Đơn Giá (VND)' },
          { key: 'thanhTien', label: 'Thành Tiền (VND)' },
        ]}
        data={data
          .map((item) =>
            item.luongGas.map((gas: any) => ({
        ...
        loading={loading}
      />
      {!loading && hasMore && data.some((item) => item.luongGas.length > 0) && (
        <button
          onClick={loadMore}
          style={{
            marginTop: '20px',
