...
const DonGiaHoi: React.FC<Props> = ({ projectId, filterDate }) => {
    ...
    <div>
      <SharedTable
        columns={[
          ...
          { key: 'luongSuDung', label: 'Lượng Hơi Sử Dụng (kg)' },
          { key: 'doanhThuBienDoi', label: 'Doanh Thu Biến Đổi (VND)' },
          { key: 'doanhThuCoDinh', label: 'Doanh Thu Cố Định (VND)' },
          { key: 'tongDoanhThu', label: 'Tổng Doanh Thu (VND)' },
        ]}
        data={data
          .map((item) =>
            item.luongHoi.map((luong: any) => ({
